...
def main(msg: func.ServiceBusMessage):
    ...
    passowrd = os.environ["POSTGRES_PW"]
    
    db_connection = psycopg2.connect(url, database, user, passowrd)
    logging.info(f'Connection to to the {database} is successfull')
    

    try:
        # TODO: Get notification message and subject from database using the notification_id
        cursor = db_connection.cursor()
        cursor.execute(f"SELECT message, subject FROM notification WHERE id={str(notification_id)}")
        logging.info(f"NotificationID {str(notification_id)}: Get message and subject")
        ...
        for row in cursor.fetchall():
            ...
            logging.info(f"ID {str(notification_id)}: First Name: {first_name}, Last Name: {last_name}, E-mail: {email}")
